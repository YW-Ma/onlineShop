...
public class ProductController {
  ...
  public ModelAndView getAllProducts() {
    ...
    // 理解一下，哪个是model，哪个是view里面的。并且找到具体的位置。
  }
  ...
  public String addProduct(@ModelAttribute Product product, BindingResult result) {
    if (result.hasErrors()) { // 表单有错，则去addProduct重新添加
      return "addProduct";
    }
    productService.addProduct(product);
    return "redirect:/getAllProducts"; // 加完后行为是redirct到所有products
  }
  ...
  public ModelAndView getEditForm(@PathVariable(value = "productId") int productId) {
    ...
    modelAndView.setViewName("editProduct"); // edit的时候，为了能告诉用户我修改了什么。
